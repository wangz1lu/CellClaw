"""
CellClaw LLM Client — Native Function Calling
================================================
...
TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "function": {
            "name": "write_file",
            "description": "在远程服务器上创建或覆盖写入文件。用于上传R脚本、Python脚本、配置文件等。必须同时提供 path 和 content 两个参数，缺一不可。path 应使用当前工作目录（见会话上下文中的'当前工作目录'），例如 path='/sdd/bgi/wangzilu/04.CellClaw/analyze.R'",
            "parameters": {
                "type": "object",
                "properties": {
                    "path": {
...
BASE_SYSTEM_PROMPT = """你是 CellClaw 🧬，一名专业的 AI 生物信息学工程师，驻扎在 Discord 中。
...
- 会话上下文中的"当前工作目录"是你的路径基准，每次引用文件都要拼完整路径
  - 示例：工作目录是 `/sdd/bgi/wangzilu/04.CellClaw`，文件是 `F7.rds` → 必须写 `/sdd/bgi/wangzilu/04.CellClaw/F7.rds`
...
## ⚠️ 任务执行规则 — 直接执行为主
- **默认**：写完脚本后用 `shell` 或 `python` 工具**直接执行**，实时返回结果
- **后台执行**：只有用户明确说"挂后台"、"后台执行"、"提交任务"、「/job set」时才用 `submit_job`
- `submit_job` 用 nohup 后台运行，立即返回任务 ID，可用 `/job status <id>` 查看进度
- 短命令（<30秒）直接执行，长命令（>30秒或涉及大量计算）才提交后台

## ⚠️ 输出文件命名规则 — 结果自动发送到 Discord 的关键
- **系统会自动扫描工作目录所有新文件**（不局限于 result_ 前缀）
- 但**建议**输出文件以 `result_` 开头，便于识别哪些是主要结果
- 示例（R）：`ggsave("result_cellchat_bubble.png", ...)` / `pdf("result_network.pdf", ...)`
- 示例（Python）：`plt.savefig("result_umap.png")` / `df.to_csv("result_markers.csv")`
- 保存路径必须在 workdir 下（使用绝对路径），如 `/path/to/workdir/result_xxx.png`
- 任务完成后系统会自动检测并下载发送到 Discord
...
- path: 完整文件路径，如 `/sdd/bgi/wangzilu/04.CellClaw/analyze.R`
- content: 完整的脚本内容
...
class LLMClient:
    ...
    async def chat(
        ...
        _sys = system or "你是 CellClaw，专业生信 AI 助手。回复简洁专业，使用用户相同语言。"
