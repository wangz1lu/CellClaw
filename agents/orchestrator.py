...
class OrchestratorAgent:
    ...
    async def _classify_intent(self, message: str, user_id: str) -> dict:
        ...
        prompt = (
            "分析用户消息，判断任务类型：\n\n"
            "【判断标准 - 按预估耗时】\n"
            "- conversation: 问好、谢谢、再见、闲聊（<1分钟）\n"
            "- status: 问当前状态（<1分钟）\n"
            "- help: 问怎么用（<1分钟）\n"
            "- query: 问信息（<1分钟）\n"
            "- file_operation: 单个文件操作：新建/删除/查看（<2分钟）\n"
            "- easy_task: 单命令/短时任务：ls/pwd/wget/curl（<3分钟）\n"
            "- analyze: 多步骤/耗时任务：分析/流程/处理/计算/可视化（>5分钟）\n\n"
            "【关键判断】\n"
            "- 包含'分析/处理/计算/跑/做/生成报告/可视化' -> analyze\n"
            "- 包含'差异分析/Seurat/Scanpy/细胞注释/富集分析' -> analyze\n"
            "- 单个ls/pwd/cd/查看文件 -> easy_task\n"
            "- 新建/删除文件夹 -> file_operation\n\n"
            "用户消息: " + message + "\n\n"
            "只返回: handler=xxx\n"
        )
